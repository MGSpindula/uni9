...
function createFourWayJunctionGraph() {
...
function createMovementDetourGraph() {
...
function createConnectionMovement(
    system,
    {
        nodeId,
        entryFromId,
    ...
    const catalog =
        system.nodeMovements;

    const resolvedEntryLaneIndex =
        Number.isInteger(
            entryLaneIndex
        )
...
test(
    "completed interaction exit releases action and approach ownership",
    () => {
...
test(
    "denied endpoint arrival keeps the incoming lane occupied",
    () => {
        ...
        /*
         * O bloqueador aparece no nó depois
         * que o ator já entrou na lane.
         */
        const blocker =
            harness.addActor(
                "Endpoint blocker",
...
test(
    "endpoint arrival uses one stable movement request before occupying the node",
    () => {
        ...
        /*
         * Autoriza a entrada na conexão a -> b.
         */
        assert.equal(
            actor.authorizeMovementTraffic(),
        ...
        /*
         * canAcceptArrival() já deve ter
         * criado a transit reservation para
         * o mesmo movement.
         */
        const nodeState =
            harness.system
        ...
        /*
         * Nenhuma segunda chamada pode trocar
         * o movement e revogar o grant.
         */
        assert.equal(
            harness.system.traffic
                .arrivals
...
test(
    "successful endpoint handoff occupies the node before releasing the incoming lane",
    () => {
...
test(
    "failed endpoint handoff does not release the incoming lane",
    () => {
        ...
        /*
         * Simula uma invalidação ocorrida
...
test(
    "compatible right turns may occupy the same junction",
    () => {

        const harness =
            createHarness(
                createFourWayJunctionGraph()
            );
        ...
        /*
         * Vindo do norte e virando para oeste.
         */
        const firstMovement =
            createConnectionMovement(
                harness.system,
                {
                    nodeId:
                        "junction",
        ...
        /*
         * Vindo do sul e virando para leste.
         *
         * É uma conversão à direita no lado
         * oposto do cruzamento.
         */
...
test(
    "perpendicular lane conflict backsteps without pushing either actor sideways",
    () => {
        ...
        winner.object3D.position.set(
            0.3,
            0,
            0
        );

        yielding.object3D.position.set(
            0,
            0,
            0.3
        );

        yielding.locomotion.activeCurve =
            curve;

        yielding.locomotion.curveDistance =
            2.3;
        ...
        /*
         * Backstep perpendicular não cria
         * deslocamento lateral em X.
         */
        assert.ok(
            Math.abs(
                yielding.object3D
        ...
        /*
         * O ator recua pela própria curva.
         */
        assert.ok(
            yielding.locomotion
                .curveDistance <
...
test(
    "stale node evacuates only the yielder through its local exit",
    () => {
        ...
        /*
         * Coloca fisicamente ambos no mesmo
         * espaço, mas registra somente o winner
         * como ocupante legítimo do nó.
         *
         * O segundo representa um overlap
         * residual que o failsafe detectou.
         */
        for (
            const actor of [
                first,
        ...
        /*
         * Faz o recovery preferir devolver cada
         * ator para o lado de onde chegou.
         */
        harness.system
        ...
        /*
         * Apenas o yielder recebe uma rota
         * de evacuação.
         */
        assert.equal(
            second.navigation
        ...
        /*
         * O winner continua como ocupante
         * legítimo até o yielder se afastar.
         */
        assert.equal(
            state.occupants
                .has(first),
        ...
        /*
         * O collisionBlock não é removido
         * prematuramente. Ele permanece até a
         * separação física dos atores.
         */
        assert.equal(
...
test(
    "collision waiting preserves the route before its stale timeout",
    () => {
        ...
        yielder.object3D.position.set(
            0.35,
            0,
            0
        );

        agent.intent.position =
            target;

        yielder.followWaypoints([
            {
                id:
                    "a",
...
test(
    "persistent collision intent survives timeout and requests separation",
    () => {
...
test(
    "stale predicted collision preserves the route and requests physical separation",
    () => {
        ...
        /*
         * Remove a semântica de nó deste teste.
         * Queremos testar o fallback off-node.
         */
        harness.system
            .trafficState
            .releaseAgent(
...
test(
    "stale replaceable collision preserves route and traffic claims while separating",
    () => {
        ...
        /*
         * Recovery físico não descarta a rota.
         */
        assert.equal(
            yielder.navigation
                .hasPath(),
        ...
        /*
         * A fila também não é cancelada por
         * um timeout de colisão.
         */
        assert.equal(
            harness.system
                .traffic
...
test(
    "collision backstep expands only until release clearance is reached",
    () => {
        ...
        /*
         * Permite que o solver alcance o
         * clearance exigido.
         */
        for (
            let frame = 0;
        ...
        /*
         * Quando o winner se afasta, a
         * manobra é imediatamente encerrada.
         */
        blocker.object3D.position.x =
            3;
...
test("completed locomotion curve stays cached across repeated endpoint checks", () => {
...
test(
    "absolute passage shares lane flow and displaces node movement claims",
    () => {
        ...
        player.navigationPassagePolicy =
            "absolute";
...
test(
    "junction movement scheduler serializes conflicts and grants independent turns",
    () => {
        ...
        /*
         * Mesmo portal de saída de firstMovement.
         * Deve conflitar.
         */
        const conflictingMovement =
            createConnectionMovement(
                harness.system,
                {
                    nodeId:
        ...
        /*
         * Conversão à direita no lado oposto.
         * Deve ser compatível.
         */
        ...
        /*
         * Primeiro movement recebe reserva
         * transitória do nó.
         */
        assert.equal(
            traffic.canCrossNode(
                "junction",
                first
            ),
        ...
        /*
         * Atualiza o pedido do segundo ator.
         *
         * NavigationDepartureQueue.enqueue()
         * deve revogar o grant anterior quando
         * movement.id muda.
         */
        traffic.arrivals.enqueue(
            "junction",
...
test(
    "movement-aware planning keeps a compatible active junction movement",
    () => {

        const harness =
            createHarness(
                createMovementDetourGraph()
            );
        ...
        /*
         * South -> east é compatível com
         * north -> west.
         */
        const path =
            harness.system.pathfinder
                .findPreferredPath(
                    "south",
                    "east",
                    actor
...
test(
    "movement-aware planning avoids an active conflicting junction movement",
    () => {
        ...
        /*
         * North -> south atravessaria o mesmo
         * centro, então deve usar o desvio.
         */
        const path =
            harness.system.pathfinder
                .findPreferredPath(
...
function assertNavigationInvariants({ graph, connector, system }) {
    ...
    for (
        ...
        assert.ok(
            ...
            `node ${node.id} has no activeMovements map`
        );

        assert.ok(
            nodeState.reservedMovements
            instanceof Map,

            `node ${node.id} has no reservedMovements map`
        );

        const ordinaryOccupants =
            [...nodeState.occupants]
                .filter(actor =>
                    !nodeState
                        .activeMovements
        ...
        assert.ok(
            ordinaryOccupants.length <= 1,

            `node ${node.id} has multiple ordinary occupants`
        );

        /*
         * Um ocupante comum, parado no nó, é
         * exclusivo.
         */
        if (
            ordinaryOccupants.length > 0
        ) {

            assert.equal(
                nodeState.activeMovements.size,
                0,

                `node ${node.id} mixes an ordinary occupant with active movements`
            );

            assert.equal(
                nodeState.reservedMovements.size,
                0,

                `node ${node.id} mixes an ordinary occupant with reserved movements`
            );
        ...
        for (
            ...
            assert.ok(
                ...
                `${actor.name} crosses node ${node.id} without occupying it`
            );
            ...
            assert.ok(
                ...
                `${actor.name} crosses node ${node.id} without an active movement`
            );
        ...
        for (
            ...
            assert.ok(
                ...
                `${actor.name} has an active movement without occupying node ${node.id}`
            );
            ...
            assert.ok(
                ...
                `${actor.name} has an active movement but is not a crossing agent`
            );
            ...
            assert.equal(
                ...
                `${actor.name} has a movement registered on the wrong node`
            );
        ...
        for (
            ...
            assert.ok(
                ...
                `${actor.name} reserves node ${node.id} without a movement`
            );
        ...
        for (
            ...
            assert.ok(
                ...
                `${actor.name} has a reserved movement without a transit reservation`
            );
            ...
            assert.equal(
                ...
                `${actor.name} has a reserved movement registered on the wrong node`
            );

        }

        /*
         * Movimentos ativos e concedidos formam
         * um conjunto compatível dois a dois.
         */
        ...
        for (
            ...
            for (
                ...
                assert.equal(
                    ...
                    `${firstActor.name} and ${secondActor.name} ` +
                    `own conflicting movements on node ${node.id}`
                );
...
test(
    "actors travelling in the same direction share one lane up to capacity",
    () => {
...
test(
    "exclusive stop movements cannot both reserve the same node",
    () => {
...
test("replaceable NPC abandons a blocked interaction and excludes it once", () => {
    ...
    assert.equal(
        npc.navigationAvoidInteractionPoint
            ?.id,
    ...
    assert.equal(
        npc.navigationAvoidInteractionPointId,
    ...
    assert.ok(request.excludePointIds.includes("blocked"));
    assert.equal(npc.navigationAvoidInteractionPoint, null);
    assert.equal(npc.navigationAvoidInteractionPointId, null);
    harness.dispose();
...
test(
    "action exit keeps action and approach claims together while graph admission waits",
    () => {
...
test(
    "predicted conflict brakes the yielder without creating a solver maneuver",
    () => {
        ...
        /*
         * Os corpos ficam suficientemente
         * próximos para previsão, mas não
         * sobrepostos.
         *
         * Com raio 0.36 para cada ator e
         * safetyPadding aproximado de 0.08,
         * overlap físico começa perto de 0.80.
         */
        first.object3D.position.set(
            0,
            0,
            0
        );

        second.object3D.position.set(
            0.9,
            0,
            0
        );

        first.followWaypoints([
            {
                id:
                    null,
        ...
        /*
         * O winner continua. O yielder freia.
         */
        assert.equal(
            firstMayMove,
            true
        ...
        /*
         * Previsão não deve criar manobra
         * física no solver.
         */
        assert.equal(
            harness.system
