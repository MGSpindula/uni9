...
export class AnimationController {
    ...
    update(delta, motion = null) {
        ...
        const targetSpeed = motion?.moving ? motion.normalizedSpeed : 0
        ...
        if (this.visualSpeed > 0.001) {
            ...
            this.walkPhase +=
                ((motion?.distanceMoved ?? 0) / this.strideLength) * Math.PI * 2

            // A small bob and lean make the primitive communicate "walking".
            this.visual.position.y =
                this.baseY +
                Math.abs(Math.sin(this.walkPhase * 2)) * 0.08 * this.visualSpeed
